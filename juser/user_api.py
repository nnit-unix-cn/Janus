...
def ssh_login(cmd):
    ...
    ssh.connect("janus",22,"root", key_filename='/root/.ssh/id_rsa')
...
def server_add_user(username, ssh_key_pwd=''):
    ...
    bash('scp -P22 /etc/passwd root@janus:/etc/passwd')
    bash('scp -P22 /etc/shadow root@janus:/etc/shadow')
    bash('scp -P22 /etc/group root@janus:/etc/group')
